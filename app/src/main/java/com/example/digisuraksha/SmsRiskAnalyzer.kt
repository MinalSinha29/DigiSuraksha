...
object SmsRiskAnalyzer {
    ...
    data class AnalysisResult(
        ...
        // Used by SmsReceiver for the notification banner text
        fun toNotificationMessage(): String = when (level) {
            ThreatLevel.CRITICAL -> "🚨 HIGH RISK: ${tags.joinToString(", ")} — Do NOT click links or share any details."
            ThreatLevel.WARNING  -> "⚠️ SUSPICIOUS SMS: ${tags.joinToString(", ")} — Stay cautious."
            ThreatLevel.INFO     -> "🔐 OTP Received — Never share this with anyone, including bank officials."
            ThreatLevel.SAFE     -> ""
        }
    }

    fun analyzeSms(text: String): AnalysisResult {
